...
apt install git libdbus-1-dev pkg-config -y 2>/dev/null || true
...
# 复制安装脚本
if [ -f "$SCRIPT_DIR/../installs/linux_install.sh" ]; then
    cp "$SCRIPT_DIR/../installs/linux_install.sh" "$COMPOSE_DIR/scripts/"
    echo "Install script copied to: $COMPOSE_DIR/scripts/linux_install.sh"
fi
...
echo "  2. Run: cd $COMPOSE_DIR && sudo ./scripts/linux_install.sh"
echo ""
